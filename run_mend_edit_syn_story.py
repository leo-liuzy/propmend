...
def run(config):
    ...
    if config.test_data == "4K_test_id":
        edit_dev_dataset = load_jsonlines(
            f"{vars.DATA_DIR}/4K_story_propagation/test_text_data_id_entity152_rel31.jsonl"
        )
        config.val_steps = 500
        assert len(edit_dev_dataset) == config.val_steps
    elif config.test_data == "4K_test_ood":
        edit_dev_dataset = load_jsonlines(
            f"{vars.DATA_DIR}/4K_story_propagation/test_text_data_ood_entity37_rel7.jsonl"
        )
        config.val_steps = 350
        assert len(edit_dev_dataset) == config.val_steps
    elif config.test_data == "4K_test_ood-relation":
        edit_dev_dataset = load_jsonlines(
            f"{vars.DATA_DIR}/4K_story_propagation/test_text_data_ood-relation_entity152_rel7.jsonl"
        )
        config.val_steps = 350
        assert len(edit_dev_dataset) == config.val_steps
    elif config.test_data == "4K_test_ood-entity":
        edit_dev_dataset = load_jsonlines(
            f"{vars.DATA_DIR}/4K_story_propagation/test_text_data_ood-entity_entity37_rel31.jsonl"
        )
        config.val_steps = 350
        assert len(edit_dev_dataset) == config.val_steps
