...
def run(config):
    ...
    if config.test_data == "4K_test_id":
        val_data = utils.load_jsonlines(f"{vars.DATA_DIR}/4K_story_propagation/test_text_data_id_entity152_rel31.jsonl")
        config.val_steps = 500
        assert len(val_data) == config.val_steps
    elif config.test_data == "4K_test_ood":
        val_data = utils.load_jsonlines(f"{vars.DATA_DIR}/4K_story_propagation/test_text_data_ood_entity37_rel7.jsonl")
        config.val_steps = 350
        assert len(val_data) == config.val_steps
    elif config.test_data == "4K_test_ood-relation":
        val_data = utils.load_jsonlines(f"{vars.DATA_DIR}/4K_story_propagation/test_text_data_ood-relation_entity152_rel7.jsonl")
        config.val_steps = 350
        assert len(val_data) == config.val_steps
    elif config.test_data == "4K_test_ood-entity":
        val_data = utils.load_jsonlines(f"{vars.DATA_DIR}/4K_story_propagation/test_text_data_ood-entity_entity37_rel31.jsonl")
        config.val_steps = 350
        assert len(val_data) == config.val_steps
